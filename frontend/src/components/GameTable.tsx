...
const GameTable: React.FC = () => {
    ...
    const fetchSeasons = useCallback(() => {
        axios.get<Season[]>('http://127.0.0.1:8000/seasons')
            .then(response => {
    ...
    const fetchSeasonTeams = useCallback(() => {
        ...
        axios.get<SeasonTeam[]>(`http://127.0.0.1:8000/season/teams?season_id=${selectedSeason}`)
            .then(response => {
    ...
    const fetchSeasonGames = useCallback(() => {
        ...
        axios.get<Game[]>(`http://127.0.0.1:8000/season/games?${queryString}`)
            .then(response => {
